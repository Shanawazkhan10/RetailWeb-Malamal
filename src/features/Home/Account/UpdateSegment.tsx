import { GetFullName } from "./SegmentDetailsSlice";

const UpdateSegment = (props: any) => {
  var ActivateSegments: Array<string> = props.addsegments;
  var DeactivateSegment: Array<string> = props.removesegments;

  var Activateflag: boolean = false;
  var Deactivateflag: boolean = false;

  if (ActivateSegments.length > 0) Activateflag = true;
  if (DeactivateSegment.length > 0) Deactivateflag = true;
    ...
    <div className="modal-body">
      ...
      <form className="edit_bank_form" style={{ display: "block" }}>
        ...
        <div className="row" style={{ display: "block" }}>
          <div className="five columns" style={{ width: "35%", float: "left" }}>
            ...
            {Deactivateflag && (
              <div className="manage-segment-head">
                ...
                <ul className="DeActivateSegments">
                  {DeactivateSegment.map((li) => (
                    <li style={{ display: "list-item", paddingBottom: "2px" }}>
                      {GetFullName(li)}
                    </li>
                  ))}
          ...
          {Deactivateflag && (
            <div
              className="Deactivate-statement"
              style={{ display: "initial" }}
            >
              <p className="text-small text-grey">
                Once deactivated, segments can only be reactivated after 12
                hours and all orders triggered by GTT / Basket / Stock SIP will
                be rejected. You will not be able to trade in the deactivated
                segments until they are reactivated.
              </p>
            </div>
          )}
        </div>
